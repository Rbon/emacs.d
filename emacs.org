...
The interaction between the previous two axioms sometimes results in the unpleasant side-effect of my code not being as [[https://en.wikipedia.org/wiki/Don%27t_repeat_yourself][dry]] as I would normally like it to be.

Let's look at an example. Say I have a package, and its related configuration.
...
Now I want to document why I use those variables, and instead of just putting one description for everything above the snippet, I split it into smaller snippets and individually document everything, like so:
...
The previous four snippets will tangle out to a file just fine, but what if someone didn't want the last part about apples, and only copied the first three snippets? They would end up with an =End of file during parsing= error. Now, this is an incredibly small example, and even a novice would likely be able to figure out what went wrong and add a closing parenthesis, but if each code snippet were longer, or more complex, or if the depth of nested parens was high, then suddenly the error becomes much more of a headache to debug. And this doesn't just apply to some hypothetical stranger finding these snippets on github. If I myself want to remove or reorder one or more snippets, then I would get errors as well.
