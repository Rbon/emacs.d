...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC a") '("applications" . user-applications-map))
  (defalias 'user-applications-map (make-sparse-keymap))
  (define-key 'user-applications-map (kbd "d") 'dired)
  (define-key 'user-applications-map (kbd "m") 'magit)
  (define-key 'user-applications-map (kbd "e") 'elfeed)
#+end_src

** Files
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC f") '("file" . user-file-map))
  (defalias 'user-file-map
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC F") '("frame" . user-frame-map))
  (defalias 'user-frame-map
    (let ((map (make-sparse-keymap)))
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC b") '("buffer" . user-buffer-map))
  (defalias 'user-buffer-map
    (let ((map (make-sparse-keymap)))
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC e") '("eval" . user-eval-map))
  (defalias 'user-eval-map
    (let ((map (make-sparse-keymap)))
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC n") '("narrow" . user-narrow-map))
  (defalias 'user-narrow-map (make-sparse-keymap))
  (define-key 'user-narrow-map (kbd "s") '("narrow to selection" . narrow-to-region))
  (define-key 'user-narrow-map (kbd "w") '("widen" . widen))
#+end_src

** Selection
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC s") '("selection" . user-selection-map))
  (defalias 'user-selection-map (make-sparse-keymap))
  (define-key 'user-selection-map (kbd "a") '("select all" . mark-whole-buffer))
  (define-key 'user-selection-map (kbd "e") '("eval selection" . eval-region))
  (define-key 'user-selection-map (kbd "i") '("indent selection" . indent-region))
#+end_src

** Cursors
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC C") '("cursor" . user-cursor-map))
  (defalias 'user-cursor-map
    (let ((map (make-sparse-keymap)))
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC w") '("window" . user-window-map))
  (defalias 'user-window-map
    (let ((map (make-sparse-keymap)))
      (define-key map (kbd "c") '("close this window" . delete-window))
      (define-key map (kbd "f") '("fullscreen this window" . delete-other-windows))
      (define-key map (kbd "H") '("split this window horizontally" . split-h-and-change-focus))
      (define-key map (kbd "V") '("split this window vertically" . split-window-vertically))
      (define-key map (kbd "h") '("move window focus left" . windmove-left))
      (define-key map (kbd "l") '("move window focus right" . windmove-right))
      (define-key map (kbd "k") '("move window focus up" . windmove-up))
      (define-key map (kbd "j") '("move window focus down" . windmove-down))
      map)
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC q") '("quit" . user-quit-map))
  (defalias 'user-quit-map
    (let ((map (make-sparse-keymap)))
      (define-key map (kbd "q") '("quit emacs" . save-buffers-kill-terminal))
      map)
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC j") '("jump" . user-jump-map))
  (defalias 'user-jump-map
    (let ((map (make-sparse-keymap)))
      (define-key map (kbd "d") '("jump to definition" . find-function))
      map)
    "Jump-related bindings.")
#+end_src

** Configuration
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC c") '("config" . user-config-map))
  (defalias 'user-config-map
    (let ((map (make-sparse-keymap)))
      (define-key map (kbd "a") '("load all configs" . rbon-load-config))
      (define-key map (kbd "b") '("bootstrap" . bootstrap))
      (define-key map (kbd "c") '("go to config" . rbon-goto-config))
      (define-key map (kbd "k") '("go to keybinds" . rbon-goto-keybinds))
      (define-key map (kbd "m") '("go to misc config" . rbon-goto-misc))
      (define-key map (kbd "f") '("go to functions" . rbon-goto-functions))
      (define-key map (kbd "p") '("to go packages" . rbon-goto-packages))
      (define-key map (kbd "i") '("to go init" . rbon-goto-init))
      map)
    "Config-related bindings.")
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC i") '("input" . user-input-map))
  (defalias 'user-input-map (make-sparse-keymap))

  (define-key 'user-input-map (kbd "t")
...
** Keybinds
I have learned some important things regarding binds that I feel must be written down, lest I forget them in the morning. Using the method in the source block below, I can append new binds to existing keymaps, while keeping the new binds local only to a buffer. It doesn't matter what the name of the map is. When two maps share the same prefix binding, their contents are somehow "merged." When I make a new binding using this method, =which-key= seems to take that new binding as the name, which means that I have the freedom to change the mnemonic of prefixes on a buffer by buffer basis, should that ever become something I want to do.
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/org.el
  (add-hook 'org-mode-hook 'my-thing)
  (defun my-thing ()
    (defalias 'user-org-mode-narrow-map (make-sparse-keymap))
    (evil-local-set-key 'normal
                        (kbd "SPC n") '("narrow" . user-org-mode-narrow-map))
    (define-key 'user-org-mode-narrow-map
      (kbd "t") '("narrow to subtree" . org-narrow-to-subtree))

    (defalias 'user-org-mode-selection-map (make-sparse-keymap))
    (evil-local-set-key 'normal
                        (kbd "SPC s") '("selection" . user-org-mode-selection-map))
    (define-key 'user-org-mode-selection-map
      (kbd "b") '("make bold" . make-bold))
    (define-key 'user-org-mode-selection-map
      (kbd "w") '("count words in selection" . count-words-region))


    (defalias 'user-org-mode-org-map (make-sparse-keymap))
    (evil-local-set-key 'normal
                        (kbd "SPC o") '("org mode" . user-org-mode-org-map))
    (define-key 'user-org-mode-org-map
      (kbd "s") '("scedule a task" . org-schedule))
    (define-key 'user-org-mode-org-map
      (kbd "d") '("set a deadline" . org-deadline))
    (define-key 'user-org-mode-org-map
      (kbd "a") '("open the agenda" . org-agenda)))
#+end_src
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (add-hook 'emacs-lisp-mode-hook 'something)
  (defun something ()
      (defalias 'user-emacs-lisp-mode-buffer-map (make-sparse-keymap))
    (evil-local-set-key 'normal
                        (kbd "SPC b") '("buffer" . user-emacs-lisp-mode-buffer-map))
    (define-key 'user-emacs-lisp-mode-buffer-map
      (kbd "i") '("indent buffer" . rbon-indent-buffer)))
#+end_src
...
** Keybinds
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/eradio.el
  (defalias 'user-radio-map (make-sparse-keymap))
  (evil-define-key '(normal visual emacs) 'global
    (kbd "SPC r") '("radio" . user-radio-map))

  (define-key 'user-radio-map (kbd "p") '("play" . eradio-play))
  (define-key 'user-radio-map (kbd "s") '("stop" . eradio-stop))
  (define-key 'user-radio-map (kbd "t") '("toggle" . eradio-toggle))
#+end_src
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/workspaces.el
  (use-package eyebrowse ; because tab-bar-mode doesn't work on mac
    ...
    (defalias 'user-workspace-map (make-sparse-keymap))
    (evil-define-key '(normal visual emacs) 'global
      (kbd "SPC `") '("workspace" . user-workspace-map)
    ...
    (define-key 'user-workspace-map (kbd "TAB") '("last used workspace" . eyebrowse-last-window-config))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "l") '("next workspace" . eyebrowse-next-window-config))
    (define-key 'user-workspace-map (kbd "TAB") '("last used workspace" . eyebrowse-last-window-config))
    (define-key 'user-workspace-map (kbd "c") '("close workspace" . eyebrowse-close-window-config))
    (define-key 'user-workspace-map (kbd "r") '("rename workspace" . eyebrowse-rename-window-config))
    (define-key 'user-workspace-map (kbd "s") '("switch to workspace..." . eyebrowse-switch-to-window-config))
    (define-key 'user-workspace-map (kbd "0") '("switch to workspace 0" . eyebrowse-switch-to-window-config-0))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "1") '("switch to workspace 1" . eyebrowse-switch-to-window-config-1))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "2") '("switch to workspace 2" . eyebrowse-switch-to-window-config-2))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "3") '("switch to workspace 3" . eyebrowse-switch-to-window-config-3))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "4") '("switch to workspace 4" . eyebrowse-switch-to-window-config-4))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "5") '("switch to workspace 5" . eyebrowse-switch-to-window-config-5))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "6") '("switch to workspace 6" . eyebrowse-switch-to-window-config-6))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "7") '("switch to workspace 7" . eyebrowse-switch-to-window-config-7))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "8") '("switch to workspace 8" . eyebrowse-switch-to-window-config-8))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "9") '("switch to workspace 9" . eyebrowse-switch-to-window-config-9))
    (define-key 'user-workspace-map (kbd "h") '("previous workspace" . eyebrowse-prev-window-config))
    (define-key 'user-workspace-map (kbd "n") '("new workspace" . eyebrowse-create-window-config))
  )
#+end_src
