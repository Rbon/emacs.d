...
Aside from keys that are part of the modern paradigm, there are *two* total keybinds in this config that use a modifier besides shift. These are =s-j= and =s-k= mapped to =helm-next-line= and =helm-previous-line= respectively. I've heard whispers of being able to work around this using =hydra=, but that is a beast I don't have the courage to grapple with right now. Someday.
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/keybinds.el
  (rbon-define-key 'org-src-mode 'normal '("SPC q f" org-edit-src-exit))
#+end_src
