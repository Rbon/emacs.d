...
* Keybinds
** NEW GLOBAL KEYS
The new idea is this: define a function that defines all the keys I would want everywhere, but make the binds local, and load the function manually in every mode hook. This means a little boilerplate, but ensures that my keys override any defaults (hopefully).
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (defun rbon-global-binds ()
    (evil-define-key '(normal visual emacs) 'local
      (kbd "SPC #"  ) '("toggle code comment" . comment-or-uncomment-region)

      (kbd "SPC a"  ) '("applications" . (keymap))
      (kbd "SPC a d") 'dired
      (kbd "SPC a m") 'magit
      (kbd "SPC a e") 'elfeed

      (kbd "SPC f"  ) '("file"                    . (keymap)        )
      (kbd "SPC f f") '("find a file"             . helm-find-files )
      (kbd "SPC f s") '("save this file"          . save-buffer     )
      (kbd "SPC f r") '("recent files"            . helm-recentf    )
      (kbd "SPC f b") '("open file browser"       . magit-dired-jump)
      (kbd "SPC f c") '("open a user config file" . find-config-file)

      (kbd "SPC F"  ) '("frame"             . (keymap)               )
      (kbd "SPC F c") '("center this frame" . rbon-center-frame      )
      (kbd "SPC F m") '("toggle maximized"  . toggle-frame-maximized )
      (kbd "SPC F f") '("toggle fullscreen" . toggle-frame-fullscreen)

      (kbd "SPC b"    ) '("buffer"               . (keymap)           )
      ...
      (kbd "SPC b b"  ) '("open the buffer list" . helm-buffers-list  )
      (kbd "SPC b TAB") '("open last buffer"     . my-switch-to-buffer)
      (kbd "SPC b e"  ) '("eval this buffer"     . eval-buffer        )
      (kbd "SPC b i"  ) '("indent buffer"        . rbon-indent-buffer )

      (kbd "SPC e"  ) '("eval"             . (keymap)       )
      (kbd "SPC e e") '("eval expression"  . eval-expression)
      (kbd "SPC e b") '("eval this buffer" . eval-buffer    )
      (kbd "SPC e s") '("eval selection"   . eval-region    )
      ...
      (kbd "SPC s a") '("select all"       . mark-whole-buffer)
      (kbd "SPC s e") '("eval selection"   . eval-region      )
      (kbd "SPC s i") '("indent selection" . indent-region    )

      (kbd "SPC C"   ) '("cursor"           . (keymap)                        )
      (kbd "SPC C a" ) '("add all matches"  . evil-mc-make-all-cursors        )
      (kbd "SPC C j" ) '("add a line down"  . rbon-add-cursor-next-line       )
      (kbd "SPC C u" ) '("undo last cursor" . evil-mc-undo-last-added-cursor  )
      (kbd "SPC C n" ) '("add next match"   . evil-mc-make-and-goto-next-match)

      (kbd "SPC h"  ) '("help"               . (keymap)          )
      (kbd "SPC h h") '("describe something" . helm-apropos      )
      (kbd "SPC h l") '("list definitions"   . helm-imenu        )
      (kbd "SPC h k") '("describe a keybind" . describe-key      )
      (kbd "SPC h v") '("describe a keybind" . describe-variable )
      (kbd "SPC h K") '("search keybinds"    . helm-descbinds    )
      (kbd "SPC h f") '("describe face"      . describe-face     )
      (kbd "SPC h F") '("describe face"      . list-faces-display)

      (kbd "SPC w")   '("window"                    . (keymap)                )
      (kbd "SPC w c") '("close this window"         . delete-window           )
      (kbd "SPC w f") '("fullscreen this window"    . delete-other-windows    )
      (kbd "SPC w H") '("split window horizontally" . split-h-and-change-focus)
      (kbd "SPC w V") '("split window vertically"   . split-window-vertically )
      ...
      (kbd "SPC q"  ) '("quit"       . (keymap)                  )
      (kbd "SPC q q") '("quit emacs" . save-buffers-kill-terminal)

      (kbd "SPC j"  ) '("jump"               . (keymap)     )
      (kbd "SPC j d") '("jump to definition" . find-function)

      (kbd "SPC c"  ) '("config"            . (keymap)           )
      (kbd "SPC c a") '("load all configs"  . rbon-load-config   )
      (kbd "SPC c b") '("bootstrap"         . bootstrap          )
      (kbd "SPC c c") '("go to config"      . rbon-goto-config   )
      (kbd "SPC c k") '("go to keybinds"    . rbon-goto-keybinds )
      (kbd "SPC c m") '("go to misc config" . rbon-goto-misc     )
      (kbd "SPC c f") '("go to functions"   . rbon-goto-functions)
      (kbd "SPC c p") '("to go packages"    . rbon-goto-packages )
      (kbd "SPC c i") '("to go init"        . rbon-goto-init     )

      (kbd "SPC i"  ) '("input"               . (keymap)           )
      (kbd "SPC i t") '("toggle input method" . toggle-input-method)
      ))
#+end_src
...
** Recentf Dialog
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  ;; (defun user-recentf-dialog-binds ()
  ;;   (rbon-global-binds)
  ...
  ;; (add-hook 'recentf-dialog-mode-hook 'user-recentf-dialog-binds)
#+end_src

** =with-editor-mode=
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (defun user-with-editor-binds ()
    (rbon-global-binds)
    (evil-define-key '(normal visual emacs) 'local
      (kbd "SPC q f") '("finish editing" . with-editor-finish)
      (kbd "SPC q c") '("cancel editing" . with-editor-cancel)
      ))

  (add-hook 'with-editor-mode-hook 'user-with-editor-binds)
#+end_src

** =messages-buffer-mode=
The usual =add-hook= method doesn't work for the messages buffer, because it comes into existence before this code would be evaluated. As far as I'm concerned, this is a hack, but it works.
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (with-current-buffer "*Messages*" (rbon-global-binds))
#+end_src

** =emacs-lisp-mode=
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/3-keybinds.el
  (add-hook 'emacs-lisp-mode-hook 'rbon-global-binds)
#+end_src
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/misc.el
    ...
    ;; (setq inhibit-splash-screen nil) ; enable splash screen
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/org.el
  (defun user-org-binds ()
    (rbon-global-binds)
    (evil-define-key '(normal visual emacs) 'local
      (kbd "SPC #"  ) '("toggle code comment" . org-comment-or-uncomment-region)
      ...
      (kbd "SPC s w") '("count words"         . count-words-region             )
      ))

  (add-hook 'org-mode-hook 'user-org-binds)
#+end_src
...
* File Browsing
** Keybinds
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/file-browser.el
  ;; (use-package ranger :defer t)
  ;; (evil-define-key 'normal 'global
  ;;   (kbd "SPC a r") 'ranger)

  (add-hook 'dired-mode-hook 'rbon-global-binds)
