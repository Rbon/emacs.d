...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config/org.el
  ...
  (defun my-thing ()
    (evil-define-key 'normal 'local
      ;; this needs to LITERALLY be '(keymap)'. This is LITERALLY A MAGICAL SEQUENCE OF LETTERS THAT MAKES THIS COMMAND WORK
      (kbd "SPC o"  ) '("org mode"                 . (keymap)             )
      (kbd "SPC o s") '("scedule a task"           . org-schedule         )
      (kbd "SPC o d") '("set a deadline"           . org-deadline         )
      (kbd "SPC o a") '("open the agenda"          . org-agenda           )
      (kbd "SPC n"  ) '("narrow"                   . (keymap)             )
      (kbd "SPC n t") '("narrow to subtree"        . org-narrow-to-subtree)
      (kbd "SPC s"  ) '("selection"                . (keymap)             )
      (kbd "SPC s b") '("make bold"                . make-bold            )
      (kbd "SPC s w") '("count words in selection" . count-words-region   )
      )
    )
#+end_src
